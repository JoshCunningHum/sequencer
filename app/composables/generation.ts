...
import { response_tst } from "~/components/Sequence/Result/warnings.test";
...
export const useGeneration = (params: {
    ...
    const generate = async () => {
        ...
        // Testing
        if (import.meta.env.VITE_VALIDATION_TESTING) {
            const txt = response_tst;
            const data = PlantUMLParser.Parse(txt);
            const converted = PlantUMLConverter.Convert({ data });
