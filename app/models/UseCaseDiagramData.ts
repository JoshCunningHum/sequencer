...
export class UseCaseDiagramData extends DrawIOXML {
  ...
  createPath(
    ...
    while ((curr = queue.shift())) {
      ...
      connections
        .filter((c, i) => {
          const isValidActor = actors.some(
            (a) => a.attributes.id === c.attributes.id,
          );
        ...
        .forEach((a) =>
          onActorConnect(a.attributes.value || "", current_value),
        );
      ...
      connections
        ...
        .forEach((a) =>
          onActorConnect(current_value, a.attributes.value || ""),
        );
    }
  }
    ...
    this.pages = diagrams.map((diagram, i) => {
      ...
      const page_systems_data = systems.map((sys) => {
        ...
        const local_usecases = usecases.filter(
          (usecase) =>
            usecase.attributes.parent === sys.attributes.id ||
            isBound(sys, usecase),
        );
