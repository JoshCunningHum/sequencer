...
export class ConvertUtil {
    ...
    static yfrom(s: SequenceElement) {
        ...
        if (isSequenceMessage(s)) {
            ConvertUtil.new_y(
                s.receiver === s.sender ? "self-message" : "message"
            );
...
export const convert = (
    ...
    const actor_record: Record<
        string,
        { actor: DIOMxCell; lifeline: DIOMxCell }
    > = {};
    const actor_record_id: Record<
        number,
        { actor: DIOMxCell; lifeline: DIOMxCell }
    > = {};
    ...
    traversedf(data.elements, (item, i, depth) => {
        ...
        if (isSequenceMessage(item)) {
            ...
            const { lifeline: source_lifeline, actor: source_actor } =
                sender_record;
